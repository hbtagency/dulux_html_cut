...
var templateData;
var websiteSection = "consumer_site";
...
//Web services calls
var BASE_URL = "umbraco/index.html"; //Base URL for webservice calls
...
var FB_BASE_URL = "ColourWallFB/index.html";
var FB_POLL_URL = "errorpages/404-page-not-found.html?";
var FB_SCRIPT_URL = '../connect.facebook.net/en_US/all.js#appId=163916583677124&xfbml=1';
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////CONFIG CONSTANTS
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
$(document).ready(function () {
    ...
    $('#projects .project h3').click(function () {
        if ($(this).parent().hasClass('open')) {
            ...
            if ($.browser.version == "7.0") { $("p.html").hide().show(); } //This fixes a stupid bug in IE7 that causes this text to be hidden. "Peekaboo" bug in IE7 may possibly be resolved by adding position:relative to the container element.
        };
    });
...
function AddFBMetaTags(colourID, colourName) {
    ...
    AddMetaTag('og:site_name', 'Dulux.com.html');
...
/*global Handlebars, templateData, YT */

var DULUX = DULUX || {};

DULUX.Refresh = (function (window, $, undefined) {
	...
	init = function () {
		...
		$('body').addClass(utilities.isTouch() ? 'touch' : 'no-touch');
	...
	menu = {
		...
		bindEvents: function () {
			...
			$li.each(function (i) {
				...
				var $this = $(this), $a, $dropdown = $this.children('.dropDown');
			...
			$('>a', $cart).on('touchstart', function (e) {
			...
			$cart.children('ul').on('touchstart', function (e) {
				e.stopPropagation();
			});
	...
	homePage = {
		...
		init: function () {
			...
			$.ajax({
                type: 'POST',
                url: '/servicesb/refresh2014Service.asmx/getconsumerhomepage', 
			    contentType: 'application/json; charset=utf-8',
			    dataType: 'json',
			    data: "{'section' : '" + websiteSection + "' }",
		...
		carousel: function () {
			...
			var $slideshow = $('.flexslider'),
				$carousel,
				tmpl = Dulux.Templates['carousel'];

			$("#carousel").html(tmpl(templateData));
			...
			$slideshow.flexslider({
				...
				start: function (slider) {
					...
					$(slider.containerSelector).children('.isVideo').each(function (index, val) {

						var $this = $(this),
							$img = $this.children('img'),
							$closeBtn = $this.children('.btn-close'),
							url = $img.data('youtube'),
							id = 'youtube-' + $img.data('index'),
							$canvas = $img.next('.youtube-canvas'),
							$sliderNav = $(slider).find('.flex-control-nav'), tag, firstScriptTag, player;
						...
						$img.on('click touchstart', function () {
							...
							$this.addClass('isPlaying').closest('.slides').addClass('slides-video');

							$slideshow.flexslider('pause');

							createPlayer();

							$sliderNav.addClass('sr-only');
						...
						function resetPlayer() {
							...
							$canvas.html('').closest('.slides').removeClass('slides-video');
							$sliderNav.removeClass('sr-only');
							player.destroy();
							$canvas.html('<div id="' + id + '" />');
				...
				before: function (slider) {
					...
					$(slider.containerSelector).children('.isPlaying').each(function () {

						var $this = $(this);
						$this.removeClass('isPlaying').children('.youtube-canvas').html('').closest('.slides').removeClass('slides-video');
		...
		inspiration: function () {
			...
			var tmpl = Dulux.Templates['row'],
				data = templateData.inspiration,
				len = data.length,
				half_len = Math.ceil(len / 2),
				firstRow = data.splice(0, half_len);


			$("#row1").html(tmpl({row: firstRow }));

			$("#row2").html(tmpl({row: data }));

			$('#inspiration-load').on('click', function () {
				$('#row2').slideDown(300);
				$(this).fadeOut(300);
			});
		...
		whatsNew: function () {
			...
			var data = templateData.whatsNew,
				len = data.length,
				tmpl = Dulux.Templates['whatsnew'],
				util = utilities,
				currentIndex = parseInt($.cookie('wnCookie'), 10), num;
			...
			$("#whats-new").html(tmpl(data[num - 1]));

		},

		banner: function () {

			var data = templateData.banner,
				len = data.length,
				tmpl = Dulux.Templates['banner'],
				util = utilities,
				currentIndex = parseInt($.cookie('bannerCookie'), 10), num;
			...
			$.cookie('bannerCookie', num);

			$("#banner").html(tmpl(data[num - 1]));

			function bannerAnimation() {
				setTimeout(function () {
					var activeFrame = $('#banner .frame.active');
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							...
							setTimeout(function () {
								...
								$('#banner .frame#frame1').addClass('active');
								bannerAnimation();
							}, 3000);
						}, 3000);
					}, 3000);
...
var phase2 = {
	...
	init: function() {
		...
		var templateViewFeatured = Dulux.Templates.featured;
		var templateViewSubNav = Dulux.Templates.subNav;
		var templateViewETC = Dulux.Templates.etc;
        if (typeof templateData != "undefined") {
			//console.log(templateData);
    		$("#featured-template").html(templateViewFeatured(templateData));
    		$("#subNav-template").html(templateViewSubNav(templateData));
    		$("#etc-template").html(templateViewETC(templateData));
        }

		$(".media").mouseenter( function() {
			var _this = $(this).find("[data-mouse-over]");
			var _data = _this.data();
			if (_data != null) {
				_this.find("i").css({"color": _data.mouseOver});
			}
		}).mouseleave( function() {
			var _this = $(this).find("[data-mouse-over]");
			var _data = _this.data();
			if (_data != null) {
				_this.find("i").css({"color": "#ffffff"});
			}
		});

		var imgHeight;
		$('.media--featured .media__image img').load( function() {
			imgHeight = $(this).height();


			$('[data-chalkboard]').css({'height': imgHeight + 'px'});
		});

		$(window).resize( function() {
			$('[data-chalkboard]').css({'height': $('.media--featured .media__image img').height() + 'px'});
		});

		$('[data-chalk-bgColor]').each( function() {
			var _this = $(this);
			var _data = _this.data();
			var raw_bgColor = _data.chalkBgcolor;
			var bgColor = _data.chalkBgcolor;

			var bgColor = bgColor.substring(1);      // strip #
			...
			if (luma < 140) {
			    var test = shadeColor1(raw_bgColor, 45);
			} else {
				var test = shadeColor1(raw_bgColor, -45);
			}


			_this.find('a').css({'color': test});
		});
